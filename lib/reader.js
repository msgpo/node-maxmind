...
Reader.prototype.get = function(ipAddress) {
  ...
  return pointer ? this.resolveDataPointer(pointer.pointer) : null;
};

Reader.prototype.getWithRoutingPrefix = function(ipAddress){
    var pointer = this.findAddressInTree(ipAddress);
    if(pointer){
        let data = this.resolveDataPointer(pointer.pointer);
        data.routingPrefix = pointer.routingPrefix;
        return data;
    }else{
      return null;
    }
